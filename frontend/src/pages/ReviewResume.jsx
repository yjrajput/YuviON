  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form onSubmit={handleOnSubmit} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <input onChange={(event) => setInput(event.target.files[0])} accept='application/pdf' rows={4} className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 text-gray-600' required type='file' />

          <p className='text-xs text-gray-500 font-semibold mt-1'>Support PDF resume only</p>
